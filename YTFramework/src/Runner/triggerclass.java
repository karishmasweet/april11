package Runner;
	...
	public class triggerclass {
		...
		public static void main(String[] args) throws BiffException, IOException {
			...
			File f = new File("/Users/karishma/Desktop/Testcase.xls"); // given the path of input excel file

			Workbook wk = Workbook.getWorkbook(f);

			Sheet s = wk.getSheet(0);

			int rows = s.getRows(); // rows fatch

			ArrayList<XmlClass> al = new ArrayList<XmlClass>();

			for (int i = 1; i < rows; i++) {

				Cell c = s.getCell(4, i); // each row with 5th column cell object created.
			...
			xT.setClasses(al); // xmlTest Object

			ArrayList<XmlTest> al2 = new ArrayList<XmlTest>();

			al2.add(xT);  // store the object of xmlTest 

			xS.setTests(al2); // pass that to the xmlSuite Object
 
			ArrayList<XmlSuite> al3 = new ArrayList<XmlSuite>();
			al3.add(xS);  // stor the object of xmlSuite

			TestNG t = new TestNG();
			t.setXmlSuites(al3);  // pass that to the TestNG
			t.run();
